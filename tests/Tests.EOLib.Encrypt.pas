...
const
  InterleaveData: array[0..6] of TStringData = (
      (Input: 'Hello, World!'; Expected: 'H!edlllroo,W '),
      (
          Input: 'We''re ¼ of the way there, so ¾ is remaining.';
          Expected: 'W.eg''nrien i¼a moefr  tshie  ¾w aoys  t,heer'
      ),
      (Input: '64² = 4096'; Expected: '6649²0 4= '),
      (Input: '© FÒÖ BÃR BÅZ 2014'; Expected: '©4 1F0Ò2Ö  ZBÅÃBR '),
      (Input: 'Öxxö Xööx "Lëïth Säë" - "Ÿ"'; Expected: 'Ö"xŸx"ö  -X ö"öëxä S" Lhëtï'),
      (Input: 'Padded with 0xFFÿÿÿÿÿÿÿÿ'; Expected: 'Pÿaÿdÿdÿeÿdÿ ÿwÿiFtFhx 0'),
      (
  ...
  DeinterleaveData: array[0..6] of TStringData = (
      (Input: 'Hello, World!'; Expected: 'Hlo ol!drW,le'),
      (
          Input: 'We''re ¼ of the way there, so ¾ is remaining.';
          Expected: 'W''e¼o h a hr,s  srmiig.nnae i¾o eetywetf  re'
      ),
      (Input: '64² = 4096'; Expected: '6²=4960  4'),
      (Input: '© FÒÖ BÃR BÅZ 2014'; Expected: '©FÖBRBZ2140 Å Ã Ò '),
      (Input: 'Öxxö Xööx "Lëïth Säë" - "Ÿ"'; Expected: 'Öx öx"ët ä"-""Ÿ  ëShïL öXöx'),
      (Input: 'Padded with 0xFFÿÿÿÿÿÿÿÿ'; Expected: 'Pde ih0FÿÿÿÿÿÿÿÿFx twdda'),
      (
          Input: 'This string contains NUL'#0' (value 0) and a € (value 128)';
  ...
  FlipMsbData: array[0..6] of TStringData = (
      (Input: 'Hello, World!'; Expected: 'Èåììï¬'#$A0'×ïòìä¡'),
      (
          Input: 'We''re ¼ of the way there, so ¾ is remaining.';
          Expected:
              '×å§òå'#$A0'<'#$A0'ïæ'#$A0'ôèå'#$A0'÷áù'#$A0'ôèåòå¬'#$A0'óï'#$A0'>'#$A0'éó'#$A0'òåíáéîéîç®'
      ),
      (Input: '64² = 4096'; Expected: '¶´2'#$A0'½'#$A0'´°¹¶'),
      (Input: '© FÒÖ BÃR BÅZ 2014'; Expected: ')' + string(#$A0) + 'ÆRV'#$A0'ÂCÒ'#$A0'ÂEÚ'#$A0'²°±´'),
      (
      ...
      (
          Input: 'This string contains NUL'#0' (value 0) and a € (value 128)';
          Expected:
              'Ôèéó'#$A0'óôòéîç'#$A0'ãïîôáéîó'#$A0'ÎÕÌ'#0#$A0'¨öáìõå'#$A0'°©'#$A0'áîä'#$A0'á'#$A0'€'#$A0'¨öáìõå'#$A0'±²¸©'
      )
  );

  SwapMultiplesData: array[0..6] of TStringData = (
      (Input: 'Hello, World!'; Expected: 'Heoll, lroWd!'),
      (
          Input: 'We''re ¼ of the way there, so ¾ is remaining.';
          Expected: 'Wer''e ¼ fo the way there, so ¾ is remaining.'
      ),
      (Input: '64² = 4096'; Expected: '64² = 4690'),
      (Input: '© FÒÖ BÃR BÅZ 2014'; Expected: '© FÒÖ ÃBR BÅZ 2014'),
      (Input: 'Öxxö Xööx "Lëïth Säë" - "Ÿ"'; Expected: 'Ööxx Xxöö "Lëïth Säë" - "Ÿ"'),
      (Input: 'Padded with 0xFFÿÿÿÿÿÿÿÿ'; Expected: 'Padded with x0FFÿÿÿÿÿÿÿÿ'),
      (
          Input: 'This string contains NUL'#0' (value 0) and a € (value 128)';
